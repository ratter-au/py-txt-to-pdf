...
#     Read lines from standard input until we either hit an empty line, indicating the end of a paragraph, or until we've filled a page.
#     Wrap the paragraph and add its contents to the text to be written to the current page.
#     If there is no text to be written to the current page (no overflow from the last page and no input), there's no need to write a new page; break out of the loop.
#     If the number of lines in the current page would exceed the length of a page, truncate it, and save the remainder to be added to the start of the next page.
